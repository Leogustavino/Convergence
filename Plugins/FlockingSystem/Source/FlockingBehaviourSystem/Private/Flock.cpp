...
{
	Boid->AssignMesh(BoidSettings.BoidMesh);
	...
	Boid->SetShadowEnabled(BoidSettings.bEnableShadows);
...
void AFlock::UpdateForces(const float NewAlignmentForce, const float NewCohesionForce, const float NewSeparationForce)
{
	for (auto Boid : Boids)
	{
		Boid->SetAlignmentForce(NewAlignmentForce);
		FlockSettings.AlignmentForce = NewAlignmentForce;

		Boid->SetCohesionForce(NewCohesionForce);
		FlockSettings.CohesionForce = NewCohesionForce;

		Boid->SetSeparationForce(NewSeparationForce);
		FlockSettings.SeparationForce = NewSeparationForce;
	}
}

void AFlock::UpdateAlignmentForce(const float NewAlignmentForce)
{
	for (auto Boid : Boids)
	{
		Boid->SetAlignmentForce(NewAlignmentForce);
...
void AFlock::UpdateCohesionForce(const float NewCohesionForce)
{
	for (auto Boid : Boids)
	{
		Boid->SetCohesionForce(NewCohesionForce);
...
void AFlock::UpdateSeparationForce(const float NewSeparationForce)
{
	for (auto Boid : Boids)
	{
		Boid->SetSeparationForce(NewSeparationForce);
