...
FVector ABoid::Align()
{
	...
	if (Neighbors > 0)
	{
		const FVector AverageVelocity = SumOfVectors / Neighbors; // Get the average velocity vectors
		FlockOwner->AverageBoidLocation = AverageVelocity;
...
void ABoid::AssignMesh(UStaticMesh* NewMesh) const
{
	if (NewMesh)
	{
		...
		for (int32 i = 0; i < StaticMeshComponent->GetNumMaterials(); i++)
			StaticMeshComponent->SetMaterial(i, NewMesh->GetMaterial(i));
	}
}

void ABoid::SetShadowEnabled(const bool EnableShadow) const
{
	StaticMeshComponent->SetCastShadow(EnableShadow);
}
...
void ABoid::CalculateRotation(FVector InSteering)
{
...
void ABoid::SetBoids(const TArray<ABoid*>& InBoids)
{
	Boids = InBoids;
}
