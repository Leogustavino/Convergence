...
struct FBoidSettings
{
	...
	// Should the boids cast shadow? (Turn this off to improve simulation performance)
	UPROPERTY(EditInstanceOnly, Category = "Flock Properties")
		bool bEnableShadows;
};
...
struct FSpawnSettings
{
	...
	UPROPERTY(EditInstanceOnly, Category = "Flock Properties", meta = (ClampMin = 2, ClampMax = 500, ToolTip = "The amount of boids to spawn for this flock"))
		uint32 NumberOfBoids = 200;
 ...
 */
UCLASS(BlueprintType, HideCategories=("Rendering", "Input", "Actor", "LOD", "Cooking", "Replication", "Collision"))
class FLOCKINGBEHAVIOURSYSTEM_API AFlock final : public AActor
{
	GENERATED_BODY()
...
protected:
	...
	FORCEINLINE float GetAlignmentForce() const { return FlockSettings.AlignmentForce; }

	UFUNCTION(BlueprintPure, Category = "Flock")
	FORCEINLINE float GetCohesionForce() const { return FlockSettings.CohesionForce; }

	UFUNCTION(BlueprintPure, Category = "Flock")
	FORCEINLINE float GetSeparationForce() const { return FlockSettings.SeparationForce; }
